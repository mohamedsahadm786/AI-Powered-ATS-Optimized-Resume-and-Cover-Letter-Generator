...
def prompt_gpt4o(job_title, company_name, location, job_description, output_dir):
    ...
    - **EXECUTIVE SUMMARY**:
        - You must keep it as a single line using: \resumeSubItem{{}}{{...}} only.
        - Do NOT convert it into multiple \item bullets.
        - Do NOT expand it with unrelated content (like financials, LLM tools, etc.).
        - Only revise if there's a clear alignment with the job title or keywords — otherwise return it exactly as-is.
        - If edited, ensure it's short (2–3 lines max), ATS-optimized, and structured like a brief career summary.
        - NEVER generate or include project experience, bullet lists, skills, or unrelated achievements.
    ...
    - **SKILLS**:
            - DO NOT duplicate any skill across categories, even if names differ (e.g., SQL vs. Advanced SQL). Keep only one.
        - Preserve and reuse existing skills from the profile unless there's a strong job match.
        - If you add a job-relevant skill:
            - Check that it’s clearly present in the personal profile or resume.
            - Add **only once** in the most relevant category (e.g., “Programming”).
            - Choose the most accurate level: (Basic), (Intermediate), or (Proficient).
        - DO NOT split the same skill into two (e.g., Python and Advanced Python). Use only one with correct level.
        - NEVER fabricate skills not supported in the personal profile.
